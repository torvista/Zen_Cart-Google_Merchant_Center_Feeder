 ...
 */
  ...
  //use Admin language for info output (not feed language)
  require(DIR_WS_LANGUAGES . zen_db_prepare_input($_GET['languageAdmin']) . '/plugin_google_mc.php');
...
if (!isset($_GET['limit'])) {
    $limit = 0; // no limit
} elseif ((int)$_GET['limit'] === 0) {
    $limit = 0; // no limit
} elseif ((int)$_GET['limit'] > 0) {
    $limit = (int)$_GET['limit'];
}
//todo remove admin constant GOOGLE_PRODUCTS_START_PRODUCTS? Why have a default offset, pre-filled on Admin page?
if (!isset($_GET['offset'])) {
    $offset = 1;
} elseif((int)$_GET['offset'] === 1) {
    $offset = 1;
} elseif((int)$_GET['offset'] > 1) {
...
if ($offset >= $limit) {
    ...
    $offset = 1;
    $error_message = 'ERROR: offset cannot be greater than the limit!';//todo language define
}
...
//check output file location permissions
    ...
    $channel_description = $dom->createElement('description', $google_base->google_base_xml_sanitizer(GOOGLE_MC_PRODUCTS_FEED_DESCRIPTION));
    ...
    switch($type) {//wot no documents or news?
      case 'products':
          ...
          if (GOOGLE_PRODUCTS_DEBUG === 'true') {
              echo '<p>Valid products=' . $products_all . NL . 'Processing ' . $products_count . ' products' . ($limit === 0 ? ' (not limited)' : ' (as per limit)') . ', starting from ' . $offset . '.</p>';
...
if (GOOGLE_PRODUCTS_OUTPUT_FILENAME !== '') {
    ...
    switch (true) {
        ...
        case($limit > $products_all && $offset === 1):
            //all products
            $suffix = '';
            break;
        case($limit === 0 && $offset !== 1):
        ...
        case($limit < $products_all && $offset === 1):
            ...
            $suffix = '_1-' . $limit;
            break;
        case($offset !== 1 && $limit < ($products_all - $offset)):
            //middle to end
            $suffix = '_' . $offset . '-' . $limit;
            break;
        case($offset !== 1 && $limit > ($products_all - $offset)):
            //middle to end
            $suffix = '_' . $offset . '-end';
    ...
    $lang_suffix = $langsMultiple ? '_' . $langCodeSelect : '';
...
//eof boilerplate text
        ...
        if (GOOGLE_PRODUCTS_DEBUG === 'true') {
            $success = false;
//todo fix this
            $position = ($offset+$loop_count-1) . '/' . $products_all . ' (' . $loop_count . '/' . $limit . ')';
...
//todo fix this
            ...
            $default_google_product_category = $google_base->google_base_xml_sanitizer(GOOGLE_PRODUCTS_DEFAULT_PRODUCT_CATEGORY);
              ...
              define('GOOGLE_PRODUCTS_URLS', 'dynamic');
              switch (GOOGLE_PRODUCTS_URLS) {
                  /*case ('default'):
                      $link = zen_href_link(zen_get_info_page($products->fields['products_id']), 'cPath=' .  zen_get_generated_category_path_rev($products->fields['master_categories_id']) . '&products_id=' . $products->fields['products_id']);
                      break;*/
                  case ('dynamic_no_cpath'):
                      $link = zen_href_link(FILENAME_DEFAULT . '.php', 'main_page=' . zen_get_info_page($products->fields['products_id']) . '&products_id=' . $products->fields['products_id'], 'NONSSL', '', '', true);
                      break;
                  case ('dynamic'):
                      $link = zen_href_link(FILENAME_DEFAULT . '.php', 'main_page=' . zen_get_info_page($products->fields['products_id']) . '&cPath=' . zen_get_generated_category_path_rev($products->fields['master_categories_id']) . '&products_id=' . $products->fields['products_id'], 'NONSSL', '', '', true);
                      break;
                  case ('magic_seo'):
                      include(DIR_WS_INCLUDES . 'modules/msu_ao_2.php');
                      break;
                  default:// as per shopfront: vanilla dynamic URL or friendly url if 3rd party plugin enabled
                      $link = zen_href_link(FILENAME_DEFAULT . '.php', 'main_page=' . zen_get_info_page($products->fields['products_id']) . '&cPath=' . zen_get_generated_category_path_rev($products->fields['master_categories_id']) . '&products_id=' . $products->fields['products_id']);
              }
