...
UPDATE configuration SET configuration_title = 'Start point for processing', configuration_description = 'Start processing products from this record.<br>This number refers to the product position in the query results, (not the product id) which will depend on the sort order (id/model/name). This start position will be needed if it becomes necessary to generate more than one feed due to memory limitations/database size.<br>When the debug option is true, the feed generation process window will display the record number.<br>You may use this start point in conjunction with the Limit option or override it on the Admin page.<br>Default start value is 1.' WHERE configuration_key = 'GOOGLE_PRODUCTS_START_PRODUCTS' LIMIT 1;
