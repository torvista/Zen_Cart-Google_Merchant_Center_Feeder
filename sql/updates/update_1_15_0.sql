...
UPDATE configuration SET configuration_title = 'Limit products to process', configuration_description = 'Limit the quantity of products to process/add to the feed file.<br>For testing or due to server limitations it may be necessary to produce multiple feed files. You may use this Limit in conjunction with the Offset option or override it on the Admin page.<br>Default value is empty.' WHERE configuration_key = 'GOOGLE_PRODUCTS_MAX_PRODUCTS' LIMIT 1;

#Update GOOGLE_PRODUCTS_START_PRODUCTS: change text and default
UPDATE configuration SET configuration_title = 'Offset process start point', configuration_description = 'Start processing products from this point.<br>This number relates to the quantity of products in the feed (not the product id). When the debug option is true, the feed generation process window will display the record number for use here.<br>For testing or due to server limitations it may be necessary to produce multiple feed files. You may use this offset in conjunction with the Limit option or override it on the Admin page.<br>Default value is empty.' WHERE configuration_key = 'GOOGLE_PRODUCTS_START_PRODUCTS' LIMIT 1;
